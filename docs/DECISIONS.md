...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. M1's UI surfaces are complete (mock data). **Durable persistence (M2 / FR-4.3) is in** (`8173d7a`) — state hydrates on launch and saves on change. Remaining solo loose ends: New Chat flow (FR-2.3 / M0-4 seed contract), per-chat YOLO/thinking-level. **Then the M5 boundary** — wiring a *real* harness adapter (ACP/OpenCode) so the agent actually runs (replacing the tracer stub) needs your environment + the redirected replay-into-carrier spike; the loop will pause and involve you there. Deferred polish: Cockpit chat tabs, in-app authoring.
