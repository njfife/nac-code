...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. M1 UI + durable persistence + the solo loose ends (New Chat FR-2.3, per-chat YOLO/thinking, `9861045`) are all in — the app is a complete, tested (11), persistent prototype on **mock data**. **▶ At the M5 boundary (needs you):** wiring a *real* harness adapter — replace the tracer stub with a real **ACP/OpenCode** adapter so the agent actually runs — requires OpenCode installed + the redirected replay-into-carrier spike, plus decisions on adapter structure. **The autonomous loop paused here.** Deferred polish (solo-doable later): Cockpit chat tabs, in-app authoring, more M0 spec docs.
