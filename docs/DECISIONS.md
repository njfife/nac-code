...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. **M1's UI surfaces are complete** — Session Stats modal (FR-11, `90d6d62`) was the last. The full design is built faithfully on **mock data** (shell, 3 views, per-chat spine, model/agent pickers, ⌘K palette, Studio/Cockpit/Focus, Context Library, Changes, compaction/branching, configurations, stats); 10 tests. **Pivot:** close the remaining M0 spec items (autonomy/security, thinking-levels, new-chat, errors, file lifecycle), then **M5** — wire real harness adapters (replace the tracer stub with ACP/OpenCode) + the **M2** persistence store. Deferred polish: Cockpit chat tabs, in-app authoring.
