...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. **Compaction & branching** (FR-9, `4ad2630`) are in, with store tests (9 total). Remaining M1 polish: Session Stats modal (FR-11), context-configuration picker (FR-6.2), Cockpit chat tabs. Then close the rest of M0 (autonomy/security, thinking-levels, new-chat, errors, file lifecycle) and wire real harness adapters (M5).
