...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. M1 UI + durable persistence are in. **M5: real Claude adapter + multi-turn conversations work** (`c9ca56f`) — per-chat transcript (history persists across restart), and Claude `--resume` carries context across turns (**verified against the real binary** — turn 2 recalled a word planted in turn 1). The same-provider half of the context requirement is delivered. 17 tests; typecheck/build green. **Next:** (a) the visible gaps flagged (workspace setup, etc.); (b) **OpenCode adapter** for local models — *needs OpenCode installed + the replay-into-carrier spike*; (c) **cross-provider transcript replay** (M0-8 Part B) once ≥2 real adapters exist. **▶ Loop paused for feedback** after conversations landed. Also fixed: sandbox preload load failure (bundle @electron-toolkit/preload); composer Context pill shows the config name.
