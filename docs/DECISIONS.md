...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. Remaining M1 polish: Session Stats modal (FR-11), Compaction & branching actions (FR-9), context-configuration picker (FR-6.2), Cockpit chat tabs. Then close the rest of M0 (autonomy/security, thinking-levels, new-chat, errors, file lifecycle) and wire real harness adapters (M5).
