...
**Next step:** M1's core surfaces are in — all 3 views (chat/context/changes), inspector, model + **agent pickers** (`917c0aa`), ⌘K palette, Studio/Cockpit/Focus layouts incl. the **Cockpit rail**. M1 UI + durable persistence are in. **M5 has begun — the first REAL harness adapter works** (`1c0609e`): a chat on provider `claude` spawns the actual `claude` binary, streams `--output-format stream-json`, and renders real `AgentEvent`s (stub still used for other providers until their adapters land). 17 tests. **GUI E2E:** `npm run dev` → send a prompt in a Claude chat → real streamed response. **Next:** (a) Claude **session resume** so turns share context (FR-4.2 native fast-path); (b) the **OpenCode carrier** adapter for local models — *needs OpenCode installed + the replay-into-carrier spike*; (c) cross-provider transcript replay (M0-8 Part B) once ≥2 real adapters exist. Deferred polish: Cockpit chat tabs, in-app authoring, more M0 docs.
